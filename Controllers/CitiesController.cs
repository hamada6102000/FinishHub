...
public class CitiesController : ControllerBase
{
    ...
    {
        var (result, message, data) = await _cities.CreateAsync(req);
    ...
    public async Task<IActionResult> GetAll() =>
        Ok(ApiResponse.Success(await _cities.GetAllAsync()));

    /// <summary>Get a single city by id.</summary>
    [HttpGet("{id}")]
    public async Task<IActionResult> GetById(int id)
    {
        var dto = await _cities.GetByIdAsync(id);
    ...
    {
        var (result, message, data) = await _cities.UpdateAsync(id, req);
        return result switch
        {
